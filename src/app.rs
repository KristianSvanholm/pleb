...
impl App {
    ...
    pub fn iterate(&mut self) {
        ...
        if self.curr_task == self.tasks.len() {
            ...
            self.results.sort_unstable_by_key(|item| (item.language.to_owned(), item.task.to_owned()));
            self.running = false;
            return;
        }
